...
/*   Updated: 2018/01/29 14:01:19 by scornaz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		mouse_hook(int button, int x, int y, void *data)
{
	...
	prog->set->min_re += xx + spanx / 10;
	prog->set->max_re += xx - spanx / 10;
	prog->set->min_im += yy + spany / 10;
	prog->set->max_im += yy - spany / 10;
	prog->set->re_factor = spanx / (SIZE_X - 1);
	prog->set->im_factor = spany / (SIZE_Y - 1);
	parallelize(prog->set, prog->libx->img->data);
